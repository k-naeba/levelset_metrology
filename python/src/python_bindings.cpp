// pybind11 bindings for levelset3d_trilinear_measure, scoped to exactly what this
...
// levelset3d_trilinear_measure's own functions. Deliberately does NOT depend on
...
#include "levelset3d_trilinear_measure/levelset3d_trilinear_measure.hpp"

namespace py = pybind11;
using namespace ns_cg;

PYBIND11_MODULE(_levelset3d_trilinear_measure, m) {
  m.doc() =
  ...
  for (const auto& o : ns_ls3tm::kCubeCornerOffset)
    corner_offsets.append(py::make_tuple(o[0], o[1], o[2]));
  m.attr("CUBE_CORNER_OFFSET") = corner_offsets;

  m.def("trilinear_value", &ns_ls3tm::TrilinearValue, py::arg("v"), py::arg("x"), py::arg("y"),
        py::arg("z"), "v: the 8 cube-corner values, ordered per CUBE_CORNER_OFFSET.");

  m.def("find_trilinear_crossings", &ns_ls3tm::FindTrilinearCrossings, py::arg("v"),
  ...
  m.def("ray_triangle_intersect", &ns_ls3tm::RayTriangleIntersect, py::arg("origin"),
        py::arg("dir"), py::arg("a"), py::arg("b"), py::arg("c"),
        "The ray parameter t where segment [origin, origin+dir] crosses "
        "triangle (a,b,c), or None.");

  m.def("find_mesh_crossings", &ns_ls3tm::FindMeshCrossings, py::arg("mesh"), py::arg("origin"),
        py::arg("dir"),
